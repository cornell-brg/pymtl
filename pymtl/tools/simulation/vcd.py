...
def _writeVcdSigs(f, hierarchy):
    ...
    for inst in hierarchy:
        ...
        for n, s in sigdict.items():
            ...
            if not s._tracing:
                ...
                if curlevel == 0 and n in ['clk','reset']:
                  s._signalvalue._code = s._code
                siglist.append(s)
            # TODO: edited this
            #w = s._nrbits
            w = s.nbits
    ...
    for s in siglist:
        ...
        if s.nbits == 1:
          print >> f, "%d%s" % (s._signalvalue.uint(), s._code)
        else:
          print >> f, "b%s %s" % (s._signalvalue.bin_str(), s._code)
    print >> f, "$end"
...
class VCDUtil():
  ...
  def recurse_models(self, model, level):
    ...
    for signal in signals:
      signal._val            = signal._signalvalue.value.uint()  # TODO: temporary
