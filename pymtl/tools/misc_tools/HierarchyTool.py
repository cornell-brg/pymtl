...
def print_ports(model, out, prefix, size):
  printn = partial( print, file=out )
  printl = partial( print, file=out, end='' )
  ...
  x = 0
  while( x < list_length):
    printl( prefix + string_size(getattr(in_ports[x],'name').ljust(length),size), )
    printn( out, string_size(getattr(out_ports[x],'name'),size) )
    x = x + 1
  while(x < len(leftover_ports)):
    printn( out, pad + prefix + string_size(getattr(leftover_ports[x],'name'),size) )
    x = x + 1

  printn( old_prefix + "END PORTS of " + getattr(model, 'name') +"\n" )
...
def print_hierarchy_helper(model, out, show_ports, prefix, depth, max_depth,size):
  ...
  for m in sub_modules:
    print( prefix + string_size(getattr(m, 'name'),size)
         + " of type " + string_size(m.class_name,size), file=out )
