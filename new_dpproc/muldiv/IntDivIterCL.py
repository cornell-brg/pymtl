...
class IntDivIterCL (Model):
  ...
  def elaborate_logic( s ):
    ...
    def logic():
      ...
      if in_go:
        s.in_op    = s.in_.msg.value[s.idx.muldiv_op].uint()

        if   s.in_op == s.idx.DIV_OP or s.in_op == s.idx.REM_OP:
          s.buf_a    = s.in_.msg.value[s.idx.arg_A].int() 
          s.buf_b    = s.in_.msg.value[s.idx.arg_B].int()
        elif s.in_op == s.idx.DIVU_OP or s.in_op == s.idx.REMU_OP:
          s.buf_a    = s.in_.msg.value[s.idx.arg_A].uint() 
          s.buf_b    = s.in_.msg.value[s.idx.arg_B].uint()
      ...
      s.out.val.next = s.buf_full and (s.counter == 0)
      s.in_.rdy.next=  (s.counter == 0)
