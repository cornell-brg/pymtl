...
class MetaListConstructor( type ):
  ...
  # called, the lambda generates a list containing n instances of type cls,
  # where n is integer passed within [] brackets, and cls is the object class
  # the [] operator was called on. Any args received by the lambda are proxied
...
# MetaCollectArgs
#------------------------------------------------------------------------------
# Metaclass which collects and stores the argument names and values used in the
...
class MetaCollectArgs( MetaListConstructor ):
  ...
  # __call__
  #----------------------------------------------------------------------------
  # Called whenever a class instance is created, allowing us to capture all the
  # arguments and argument values passed to the class. These arguments are
  # stored as an OrderedDict in the _args field of the instance so they can be
  # used later.
