...
class GcdProcCtrl( Model ):
  ...
  def elaborate_logic( s ):
    ...
    def state_to_ctrl():
      ...
      a_reg_en = y if s.ctrl_msg == 1 else n
      b_reg_en = y if s.ctrl_msg == 2 else n
      #                                                         a_mux_sel   a_reg_en     b_mux_sel  b_reg_en
      if   current_state == s.STATE_IDLE:        cs = concat(a_mux_sel_in , a_reg_en, b_mux_sel_in, b_reg_en)
      elif current_state == s.STATE_LOAD:        cs = concat(a_mux_sel_in , a_reg_en, b_mux_sel_in, b_reg_en)
      elif current_state == s.STATE_CALC:
        swap = s.ss.is_a_lt_b
        done = not s.ss.is_a_lt_b and s.ss.is_b_zero
        a_reg_en = y if not done else n
        b_reg_en = y if not done else n
        if swap:                                 cs = concat(a_mux_sel_b  , a_reg_en, b_mux_sel_a , b_reg_en)
        else:                                    cs = concat(a_mux_sel_sub, a_reg_en, b_mux_sel_x ,        n)
    ...
    def ctrl_to_dpath():
      ...
      s.cs.a_mux_sel.value = s.ctrl_signals[3::4]
