...
class TestHarness (Model):
  ...
  def __init__( s, src_msgs, sink_msgs, src_delay, sink_delay,
    ...
    for i in xrange( s.num_routers ):
      ...
      connect( s.v2c[i].to_msg,        s.torus.in_msg[i]    )
      connect( s.v2c[i].to_val,        s.torus.in_val[i]    )
      connect( s.v2c[i].to_credit,     s.torus.in_credit[i] )

      connect( s.torus.out_msg[i],     s.c2v[i].from_msg         )
      connect( s.torus.out_val[i],     s.c2v[i].from_val         )
      connect( s.torus.out_credit[i],  s.c2v[i].from_credit      )
...
def nearest_neighbor_east_msgs( size ):
  ...
  for i in xrange( num_routers ):
    for j in xrange( size ):
      ...
      data_roll = random.randint( 0, pow( 2, 18 ) - 1 )
