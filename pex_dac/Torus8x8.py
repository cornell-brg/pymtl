...
class Torus8x8(Model):
  ...
  def __init__( s ):
    ...
    for i in xrange( num_routers ):
      ...
      connect( s.in_msg[i],             s.routers[i].in_msg[4]    )
      connect( s.in_val[i],             s.routers[i].in_val[4]    )
      connect( s.in_credit[i],          s.routers[i].in_credit[4] )

      connect( s.routers[i].out_msg[4],     s.out_msg[i]         )
      connect( s.routers[i].out_val[4],     s.out_val[i]         )
      connect( s.routers[i].out_credit[4],  s.out_credit[i]      )
