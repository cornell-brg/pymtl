...
class MeshRouterCL( Model ):
  ...
  def elaborate_logic( s ):
    ...
    @s.tick
    def router_logic():
      #a = [ True for x in s.output_regs   if x.data          ]
      #b = [ True for x in s.input_buffers if x.data          ]
      #c = [ True for x in s.in_           if x.rdy and x.val ]
      #if ( not a and not b and not c and not s.reset ):
      #  return
  ...
  def route_compute( s, dest ):
    ...
    x_dest = dest % s.xnodes
    y_dest = dest / s.xnodes
