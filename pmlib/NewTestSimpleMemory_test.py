...
from NewTestSimpleMemory import TestSimpleMemory
...
class TestHarness (Model):
  ...
  def __init__( self, memreq_params, memresp_params, nports,
    ...
    self.mem  = TestSimpleMemory ( memreq_params, memresp_params, nports )
  ...
  def line_trace( self ):
    #print
    #print self.src[0].out_msg.value,'->',self.mem.memreq_msg[0].value,'->', self.mem.memresp_msg[0].value
...
def run_mem_test( dump_vcd, vcd_file_name, src_delay, sink_delay, nports,
  ...
  memreq_params  = mem_struct.MemReqMsg( 32, 32 )
  memresp_params = mem_struct.MemRespMsg( 32 )
